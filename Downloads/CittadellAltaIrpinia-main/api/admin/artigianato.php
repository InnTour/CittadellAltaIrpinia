...
requireAdminSession();
requireOperatorAccess('craft');
$_su       = getAdminSessionUser();
$_isAdmin  = ($_su['role'] === 'admin');
$_entityId = $_isAdmin ? null : ($_su['company_id'] ?? null); // company_id = artisan_id
$db = getDB();
$msg = '';
...
if ($_entityId) {
    $stmtList = $db->prepare("SELECT id, name, borough_id, price FROM craft_products WHERE artisan_id = ? ORDER BY name ASC");
