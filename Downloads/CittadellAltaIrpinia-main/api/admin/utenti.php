...
// ── AUTO-MIGRATE: crea/aggiorna la tabella utenti ─────────
ensureAdminUsersTable($db); // gestita in db.php (include operator_type)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $opType  = ($role === 'operatore') ? (trim($_POST['operator_type'] ?? '') ?: null) : null;
    $borough = ($opType === 'borough') ? trim($_POST['borough_id'] ?? '') : null;
    $company = ($opType && $opType !== 'borough') ? trim($_POST['company_id'] ?? '') : null;
...
// ── LISTA + FORM ───────────────────────────────────────────
$list = $db->query("
    SELECT id, name, email, role, operator_type, is_active, last_login_at
    FROM admin_users
...
if (isset($_GET['edit'])) {
    if ($_GET['edit'] === '__new__') {
        $sel = ['id'=>'','name'=>'','email'=>'','role'=>'registrato','operator_type'=>'','phone'=>'','bio'=>'',
...
// Elenco entità per i select dinamici
...
try { $ristoranti  = $db->query("SELECT id, name FROM restaurants ORDER BY name ASC")->fetchAll(); } catch (PDOException $e) { $ristoranti = []; }
try { $ospitalita  = $db->query("SELECT id, name FROM accommodations ORDER BY name ASC")->fetchAll(); } catch (PDOException $e) { $ospitalita = []; }

$pageTitle = 'Utenti';
require '_layout.php';

$roleBadge = function(string $role, string $opType = ''): string {
    if ($role === 'operatore' && $opType) {
        $label = match($opType) {
            'borough'       => 'Op. Borgo',
            'company'       => 'Op. Azienda',
            'experience'    => 'Op. Esperienze',
            'craft'         => 'Artigiano',
            'food'          => 'Produttore Food',
            'accommodation' => 'Op. Ospitalità',
            'restaurant'    => 'Ristoratore',
            default         => 'Operatore',
        };
        return '<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-900/60 text-amber-300 border border-amber-700">' . htmlspecialchars($label) . '</span>';
    }
    return match($role) {
      ...
      <a href="utenti.php?edit=<?= urlencode($u['id']) ?>"
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="flex items-center gap-2 mt-0.5">
            <?= $roleBadge($u['role'], $u['operator_type'] ?? '') ?>
    ...
    <form method="POST" class="bg-slate-800 rounded-xl border border-slate-700 p-6 space-y-4">
      ...
      <div id="operatoreFields" style="display:<?= ($sel['role']??'')==='operatore'?'block':'none' ?>">
        <div class="border border-amber-800/50 bg-amber-900/10 rounded-lg p-4 space-y-3">
          <p class="text-xs text-amber-400 font-medium">⚙️ Tipo e assegnazione operatore</p>

          <!-- Tipo operatore -->
          <div>
            <label class="block text-xs text-slate-400 mb-1">Tipo operatore <span class="text-red-400">*</span></label>
            <select name="operator_type" id="operatorTypeSelect"
              class="w-full bg-slate-700 text-white rounded-lg px-3 py-2 text-sm border border-slate-600 focus:outline-none focus:border-emerald-500"
              onchange="updateEntitySelector(this.value)">
              <option value="">— Seleziona tipo —</option>
              <option value="borough"       <?= ($sel['operator_type']??'')==='borough'?'selected':'' ?>>🏔️ Operatore Borgo</option>
              <option value="company"       <?= ($sel['operator_type']??'')==='company'?'selected':'' ?>>🏢 Operatore Azienda</option>
              <option value="experience"    <?= ($sel['operator_type']??'')==='experience'?'selected':'' ?>>🎭 Operatore Esperienze</option>
              <option value="craft"         <?= ($sel['operator_type']??'')==='craft'?'selected':'' ?>>🏺 Artigiano</option>
              <option value="food"          <?= ($sel['operator_type']??'')==='food'?'selected':'' ?>>🧀 Produttore Food</option>
              <option value="accommodation" <?= ($sel['operator_type']??'')==='accommodation'?'selected':'' ?>>🏨 Operatore Ospitalità</option>
              <option value="restaurant"    <?= ($sel['operator_type']??'')==='restaurant'?'selected':'' ?>>🍽️ Ristoratore</option>
            </select>
            <p class="text-xs text-slate-500 mt-1">Determina a quale sezione del pannello avrà accesso l'operatore.</p>
          </div>

          <!-- Selettore entità — mostrato in base al tipo -->
          <!-- Borgo -->
          <div id="entityBoroughWrap" style="display:none">
            ...
            <select name="borough_id" class="w-full bg-slate-700 text-white rounded-lg px-3 py-2 text-sm border border-slate-600 focus:outline-none focus:border-emerald-500">
              ...
              <option value="<?= htmlspecialchars($b['id']) ?>" <?= ($sel['borough_id']??'')===$b['id']?'selected':'' ?>><?= htmlspecialchars($b['name']) ?></option>
              <?php endforeach; ?>
            </select>
          </div>

          <!-- Azienda / Esperienze / Artigianato / Food -->
          <div id="entityCompanyWrap" style="display:none">
            <label class="block text-xs text-slate-400 mb-1" id="entityCompanyLabel">Azienda assegnata</label>
            <select name="company_id" class="w-full bg-slate-700 text-white rounded-lg px-3 py-2 text-sm border border-slate-600 focus:outline-none focus:border-emerald-500">
              ...
              <option value="<?= htmlspecialchars($a['id']) ?>" <?= ($sel['company_id']??'')===$a['id']?'selected':'' ?>><?= htmlspecialchars($a['name']) ?></option>
              <?php endforeach; ?>
            </select>
            <p class="text-xs text-slate-500 mt-1">L'operatore vedrà solo le voci legate a questa azienda.</p>
          </div>

          <!-- Ospitalità -->
          <div id="entityAccommodationWrap" style="display:none">
            <label class="block text-xs text-slate-400 mb-1">Struttura ricettiva assegnata</label>
            <select name="company_id" class="w-full bg-slate-700 text-white rounded-lg px-3 py-2 text-sm border border-slate-600 focus:outline-none focus:border-emerald-500">
              <option value="">— Seleziona struttura —</option>
              <?php foreach ($ospitalita as $o): ?>
              <option value="<?= htmlspecialchars($o['id']) ?>" <?= ($sel['company_id']??'')===$o['id']?'selected':'' ?>><?= htmlspecialchars($o['name']) ?></option>
              <?php endforeach; ?>
            </select>
          </div>

          <!-- Ristorazione -->
          <div id="entityRestaurantWrap" style="display:none">
            <label class="block text-xs text-slate-400 mb-1">Ristorante/locale assegnato</label>
            <select name="company_id" class="w-full bg-slate-700 text-white rounded-lg px-3 py-2 text-sm border border-slate-600 focus:outline-none focus:border-emerald-500">
              <option value="">— Seleziona locale —</option>
              <?php foreach ($ristoranti as $r): ?>
              <option value="<?= htmlspecialchars($r['id']) ?>" <?= ($sel['company_id']??'')===$r['id']?'selected':'' ?>><?= htmlspecialchars($r['name']) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
      ...
      <script>
      function updateEntitySelector(type) {
        const wraps = ['entityBoroughWrap','entityCompanyWrap','entityAccommodationWrap','entityRestaurantWrap'];
        wraps.forEach(id => document.getElementById(id).style.display = 'none');
        const lbl = document.getElementById('entityCompanyLabel');
        if (type === 'borough') {
          document.getElementById('entityBoroughWrap').style.display = 'block';
        } else if (type === 'restaurant') {
          document.getElementById('entityRestaurantWrap').style.display = 'block';
        } else if (type === 'accommodation') {
          document.getElementById('entityAccommodationWrap').style.display = 'block';
        } else if (['company','experience','craft','food'].includes(type)) {
          document.getElementById('entityCompanyWrap').style.display = 'block';
          const labels = {company:'Azienda assegnata', experience:'Azienda fornitrice esperienze', craft:'Azienda artigiana', food:'Azienda produttrice'};
          if (lbl) lbl.textContent = labels[type] || 'Azienda assegnata';
        }
      }
      // Inizializza al caricamento
      updateEntitySelector(document.getElementById('operatorTypeSelect')?.value || '');
      </script>
