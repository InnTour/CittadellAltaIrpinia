...
requireAdminSession();
requireOperatorAccess('restaurant'); // solo admin o operatore tipo restaurant
$_su      = getAdminSessionUser();
$_isAdmin = ($_su['role'] === 'admin');
$_entityId = $_isAdmin ? null : ($_su['company_id'] ?? null);
$db = getDB();
$msg = '';
...
try {
    if ($_entityId) {
        $stmtList = $db->prepare("SELECT id, name, borough_id, type FROM restaurants WHERE id = ? ORDER BY name ASC");
