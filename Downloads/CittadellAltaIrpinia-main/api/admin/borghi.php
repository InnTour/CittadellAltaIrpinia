...
requireAdminSession();
requireOperatorAccess('borough');
$_su       = getAdminSessionUser();
$_isAdmin  = ($_su['role'] === 'admin');
$_entityId = $_isAdmin ? null : ($_su['borough_id'] ?? null);
$db = getDB();
$msg = '';
...
// ── Lista borghi ──────────────────────────────────────────
if ($_entityId) {
    $stmtBorghi = $db->prepare("SELECT * FROM boroughs WHERE id = ? ORDER BY name ASC");
    $stmtBorghi->execute([$_entityId]);
    $borghi = $stmtBorghi->fetchAll();
} else {
