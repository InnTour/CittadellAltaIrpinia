...
requireAdminSession();
requireOperatorAccess('food');
$_su       = getAdminSessionUser();
$_isAdmin  = ($_su['role'] === 'admin');
$_entityId = $_isAdmin ? null : ($_su['company_id'] ?? null); // company_id = producer_id
$db = getDB();
$msg = '';
...
try {
    if ($_entityId) {
        $stmtList = $db->prepare("SELECT id, name, borough_id, category FROM food_products WHERE producer_id = ? ORDER BY name ASC");
