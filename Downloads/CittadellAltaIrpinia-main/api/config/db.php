...
function ensureAdminUsersTable(PDO $db): void {
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `admin_users` (
      ...
      `operator_type` VARCHAR(30)   DEFAULT NULL COMMENT 'Tipo operatore: borough|company|experience|craft|food|accommodation|restaurant',
      `borough_id`    VARCHAR(100)  DEFAULT NULL,
      `company_id`    VARCHAR(100)  DEFAULT NULL COMMENT 'Usato anche come entity_id per ristoranti/ospitalità',
    ...
    // Auto-migrazione: aggiunge operator_type se la tabella esiste già senza di essa
    try {
        $db->query("SELECT `operator_type` FROM `admin_users` LIMIT 0");
    } catch (PDOException $e) {
        $db->exec("ALTER TABLE `admin_users` ADD COLUMN `operator_type` VARCHAR(30) DEFAULT NULL AFTER `role`");
    }
}
...
// Ritorna le info complete dell'utente della sessione admin corrente
function getAdminSessionUser(): array {
    return [
        'id'            => $_SESSION['admin_user_id']      ?? null,
        'name'          => $_SESSION['admin_user_name']    ?? ADMIN_USER,
        'role'          => $_SESSION['admin_role']         ?? 'admin',
        'operator_type' => $_SESSION['admin_operator_type'] ?? null,
        'borough_id'    => $_SESSION['admin_borough_id']   ?? null,
        'company_id'    => $_SESSION['admin_company_id']   ?? null,
    ];
}

// Ritorna l'etichetta leggibile del ruolo/tipo operatore corrente
function getAdminRoleLabel(): string {
    $user = getAdminSessionUser();
    if ($user['role'] === 'admin') return 'Amministratore';
    return match($user['operator_type'] ?? '') {
        'borough'       => 'Operatore Borgo',
        'company'       => 'Operatore Azienda',
        'experience'    => 'Operatore Esperienze',
        'craft'         => 'Artigiano',
        'food'          => 'Produttore Food',
        'accommodation' => 'Operatore Ospitalità',
        'restaurant'    => 'Ristoratore',
        default         => 'Operatore',
    };
}

// Ritorna la pagina di destinazione per l'operatore corrente
function getOperatorHomePage(): string {
    $user = getAdminSessionUser();
...
// Verifica che l'operatore abbia accesso alla sezione richiesta.
// Per gli admin passa sempre. Per gli operatori verifica operator_type.
// In caso di accesso negato, redirige alla pagina appropriata.
function requireOperatorAccess(string $requiredType): void {
    $user = getAdminSessionUser();
    ...
    if (($user['operator_type'] ?? '') !== $requiredType) {
        header('Location: ' . getOperatorHomePage());
        exit;
    }
}
