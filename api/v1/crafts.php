...
function buildCraft(PDO $db, array $row): array {
    ...
    $row['is_custom_order_available'] = (bool)($row['is_custom_order_available'] ?? false);
    $row['is_unique_piece']           = (bool)($row['is_unique_piece'] ?? false);
...
if ($method === 'GET') {
    if ($id || $slug) {
        if ($slug) {
            $stmt = $db->prepare("SELECT * FROM craft_products WHERE slug = ?");
            $stmt->execute([$slug]);
        } else {
...
function _craftValues(array $b): array {
    return [
        ...
        $b['price'] ?? null,
        ($b['is_custom_order_available'] ?? false) ? 1 : 0,
        ...
        ($b['is_unique_piece'] ?? false) ? 1 : 0, $b['production_series_qty'] ?? null,
